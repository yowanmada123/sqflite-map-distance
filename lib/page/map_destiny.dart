...
class DestinyMap extends StatefulWidget {
  final LatLng latlongAwal;
  final Function(String)? onChanged;
  final Function(String)? onChangedDistance;
  const DestinyMap({Key? key, this.onChanged, required this.latlongAwal, this.onChangedDistance}) : super(key: key);

  @override
  State<DestinyMap> createState() => DestinyMapState();
}

class DestinyMapState extends State<DestinyMap> {
  ...
  LatLng newLastPoint = LatLng(53.225768, -101.753958);
  ...
  static double calculateDistance(LatLng p1, LatLng p2) {
    ...
    double jarak = distance.as(LengthUnit.Meter, p1, p2) / 1000;
    return jarak;
  }
  ...
  void _handleTap(var tapPosition, LatLng latlng) {
    setState(() {
      ...
      newLastPoint = latlng;
      var jarak = calculateDistance(widget.latlongAwal, latlng);
      if (widget.onChangedDistance != null) {
        widget.onChangedDistance!("${jarak.toString()} KM");
      }
    });
  }
              ...
              MarkerLayer(
                markers: [
                  Marker(
                    width: 500.0,
                    height: 500.0,
                    point: LatLng(widget.latlongAwal.latitude, widget.latlongAwal.longitude), // Second marker position
                    builder: (ctx) => Container(
                      child: Icon(
                        Icons.location_on,
                        size: 40.0,
                  ...
                  Marker(
                    width: 500.0,
                    height: 500.0,
                    point: LatLng(newLastPoint.latitude, newLastPoint.longitude), // Second marker position
                    builder: (ctx) => Container(
                      child: Icon(
                        Icons.location_on,
                        size: 40.0,
